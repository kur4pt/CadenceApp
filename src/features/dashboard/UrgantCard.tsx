...
const urgentItems = [
    ...
    { title: 'CS problem set',   meta: 'Due today - 11:59 AM',  tone: 'Assignment' },
]
    ...
    return (
        <section className="surface-card overflow-hidden">
            <div className="flex items-center justify-between border-b border-[rgb(var(--border))] px-5 py-4">
                ...
                <span className="rounded-full bg-[rgb(var(--surface-subtle))] px-2.5 py-1 text-[11px] text-[rgb(var(--muted))]">Preview</span>
            </div>

            <div className="divide-y divide-[rgb(var(--border))]">
                {urgentItems.map((item) => (
                    <button key={item.title} className='focus-ring flex w-full items-center gap-4 px-5 py-4 text-left transition hover:bg-[rgb(var(--surface-subtle))]'>
